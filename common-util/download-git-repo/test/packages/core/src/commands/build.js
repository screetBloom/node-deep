const RomeDependence = require('../../class/rome-dependence/RomeDependence.js')
const {
  formatStats,
  resolveLibConfig,
  resolveWcConfig,
    ...
    async (args, rawArgs) => {
      ...
      if (args.modern && args.target === 'app') {
        ...
        if (!process.env.VUE_CLI_MODERN_BUILD) {
          ...
          const cliBin = require('path').resolve(__dirname, '../../bin/index.js')
...
async function build(args, api, options) {
  ...
  let webpackConfig
  if (args.target === 'lib') {
    webpackConfig = resolveLibConfig(api, args, options)
  } else if (args.target === 'wc' || args.target === 'wc-async') {
    webpackConfig = resolveWcConfig(api, args, options)
  } else {
    webpackConfig = resolveAppConfig(api, args, options)
  }
